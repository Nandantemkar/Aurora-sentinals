...
async function resolveEmailByUserId(userId: string): Promise<string | undefined> {
...
export const createSOS = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const eventWithEmail = {
      ...event,
      email: await resolveEmailByUserId(event.user_id),
    };
...
async function getChatBundleForSOS(params: { sosId: string; requester: { id: string; role: string } }) {
  ...
  const { data: senders } = senderIds.length
    ? await supabaseAdmin.from('users').select('id,email').in('id', senderIds)
    : ({ data: [] } as any);
  ...
  const securityEmail = chat.security_id ? await resolveEmailByUserId(chat.security_id) : undefined;
    ...
    if (events.length > 0) {
      ...
      if (missingIds.length > 0) {
        const resolved = await Promise.all(
          missingIds.map(async (uid) => ({ uid, email: await resolveEmailByUserId(uid) }))
        );
        resolved.forEach((r) => {
          if (r.uid && r.email) {
            emailById.set(r.uid, r.email);
          }
        });
      }
